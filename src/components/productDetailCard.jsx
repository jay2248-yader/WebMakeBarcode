...
const ProductDetailCard = ({ priceData }) => {
  ...
  // ถ้า productbarcode เป็น null ให้ใช้ object ว่าง
  const product = productbarcode?.product || {};

  // ถ้า productbarcode เป็น array หรือ object ให้ fallback เป็น array ว่าง
  ...
  return (
    <div className="backdrop-blur-lg bg-white/100 shadow-xl rounded-3xl p-8 mb-8 w-full max-w-3xl mx-auto border border-gray-100 transform transition-all duration-300 hover:scale-[1.01] hover:shadow-2xl">
      ...
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 pb-6 border-b border-gradient-to-r from-transparent via-gray-200 to-transparent">
        <div>
        ...
        <div className="space-y-4">
          {barcodes.length > 0 ? (
            barcodes.map((bc, idx) => (
              <div
                key={idx}
                className="p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl border border-gray-200 text-gray-800 text-sm flex flex-col sm:flex-row sm:items-center sm:justify-between shadow-sm hover:shadow-md transition"
              >
                <span className="font-mono text-base font-semibold">{bc.BARCODE || "ບໍ່ມີຂໍ້ມູນ"}</span>
                <span className="text-gray-500 text-xs md:text-sm mt-2 sm:mt-0">{bc.BARCODETEXT || "ບໍ່ມີຂໍ້ມູນ"}</span>
              </div>
            ))
          ) : (
