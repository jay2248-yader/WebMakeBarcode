// src/components/ProductDetailBack.jsx
import React from "react";
import { FaBarcode, FaFileInvoice, FaBalanceScale, FaWarehouse, FaTag } from "react-icons/fa";
...
const ProductDetailBack = ({ barcodeData }) => {
    ...
    return (
      <div className="backdrop-blur-lg bg-white/100 shadow-xl rounded-3xl p-8 w-full max-w-3xl mx-auto border border-gray-100">
        <div className="text-center py-12">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">No Barcode Data Available</h2>
  ...
  return (
    <div className="backdrop-blur-lg bg-white/100 shadow-xl rounded-3xl p-8 w-full max-w-3xl mx-auto border border-gray-100">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 pb-6 border-b border-gray-200">
        <div>
          <h2 className="text-3xl font-extrabold text-gray-900">{product?.NAMETH || "Unknown Product"}</h2>
      ...
      <div className="mt-8 pt-6 border-t border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {renderInfoItem(<FaWarehouse className="text-lg text-orange-600" />, "Warehouse / Location", `${product?.GRWAREHOUSE || "N/A"} / ${product?.GRLOCATION || "N/A"}`)}
          {renderInfoItem(<FaBalanceScale className="text-lg text-purple-600" />, "Stock Unit / Sale Unit", `${product?.STOCKUNIT || "N/A"} / ${product?.SALEUNIT || "N/A"}`)}
        </div>
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-gray-200">
        ...
        <div className="space-y-4 max-h-[50vh] overflow-y-auto px-1">
          {barcodeList.map((bc, idx) => (
            <div key={idx} className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-5 shadow hover:shadow-lg transition">
              <p className="font-mono font-semibold text-base">{bc.BARCODE || "N/A"}</p>
              <Barcode value={bc.BARCODE || ""} format="CODE128" width={1.5} height={50} displayValue={false} background="#fff" lineColor="#000" />
              <p className="text-gray-700 text-sm mt-2">{bc.BARCODETEXT || "N/A"}</p>
            </div>
          ))}
        </div>
      </div>
